    ...
    if (parent_idx == -1) {
      ...
      if (rstate->metrics.prefill_tokens > 0 && rstate->metrics.decode_tokens > 0) {
        ...
        if (prefill_time > 0 && decode_time > 0) {
          ...
          printf("[Request Completed] Prefill: %ld tokens, Decode: %ld tokens, Prefill TPS: %.1f, Decode TPS: %.1f\n",
                 rstate->metrics.prefill_tokens, rstate->metrics.decode_tokens, prefill_tps, decode_tps);
          fflush(stdout);
        }
      }
